...
export default function CreateButton({ disabled, user, config, onReloadPage }) {
    ...
    useEffect(() => {
        ...
        const _formData = {};
        if (config?.project) {
            _formData["project"] = config?.project;
        }
        if (config?.doc === "2_1") {
            _formData["purpose"] = "progress";
        } else if (config?.doc === "2_2") {
            _formData["purpose"] = "final";
        }

        setFormData((prev) => {
            return { ...prev, ..._formData };
        });
    }, [user, config]);
    ...
    return (
        <>
            ...
            <Modal
                ...
                show={modalShow}
                size="xl"
                aria-labelledby="contained-modal-document2-create"
                centered
            >
                <Modal.Header closeButton>
                    <Modal.Title id="contained-modal-document2-create">
                        กรอกเอกสาร ป.2
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
